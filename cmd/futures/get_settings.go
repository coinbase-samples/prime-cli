...
var getSettingsCmd = &cobra.Command{
	Use:   "get-settings",
	Short: "Gets FCM settings for an entity",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		request := &futures.GetFcmSettingsRequest{
			EntityId: entityId,
		}

		response, err := svc.GetFcmSettings(ctx, request)
		if err != nil {
			return fmt.Errorf("cannot get FCM settings: %w", err)
		}
...
func init() {
	...
	utils.AddEntityIdFlag(getSettingsCmd)
}
